...
| 招投标资料采集模块开发 | 09-09~09-11(提前至 08-12) | 🔄 进行中 | P5-01 已完成(契约 + mock 采集器,20 项新测试,对抗评审 5 项发现已全部修复);P5-02 去重台账持久化已完成(JSON Lines + DI 注册);P5-05a 已完成(robots.txt 合规缓存、三层限速器、HTTP 采集器、平台解析抽象、复合采集器与 DI 集成,14 个新文件,测试基线 314 通过/4 跳过);仅剩 P5-05b/c 真实平台 parser 待平台清单确认后逐个接入 |
...
| 2026-08-11 | **P5-02 完成**（去重台账 + JSON Lines 持久化，commit `596de05`）：新增 `IBiddingNoticeLedger`（`TryRegisterAsync` / `MarkNotifiedAsync` / `PruneAsync`）、内存实现与 `JsonLinesBiddingNoticeLedger`。台账按公告指纹去重、记录首次发现时间与推送状态，追加写入 `.jsonl` 并周期性压缩（原子 `File.Replace`）；损坏文件隔离为 `.corrupted.*`。此为规划阶段识别的"重启后重复推送"前置依赖，队列本身仍保持内存态未被拖入关键路径。 |
| 2026-08-11 | **P5-03 完成**（推送通道，commit `f697be1`，9 文件 +1144 行）：新增 `WebhookNotificationChannel` 与 `SmtpNotificationChannel`，均为"未配置即安全失败"。安全边界按 P5-00 约束落地：`Enabled` 默认 `false`、`DryRun` 默认 `true`，因此默认配置的 host **不可能**发出真实推送；DryRun 短路发生在任何 HTTP/SMTP 连接之前（测试断言 `RequestCount == 0`）；Webhook 强制 HTTPS、拒绝 IP 字面量与私有/回环/链路本地地址、DNS 解析失败即视为不安全；payload 限 100 KB。凭据只从配置读取，日志不含收件人、Webhook URL 与 SMTP 凭据。 |
| 2026-08-11 | **P5-03 安全取向差异（刻意保留，非疏漏）**：`SsrfGuard` 原有的同步邮件收件人检查在 DNS 失败时**失败开放**（返回 `true`），直接改会静默改变既有行为，因此新增失败关闭的 `IsEmailRecipientSafeAsync` 供 SMTP 通道使用，原同步方法保持原状。SMTP 对每个收件人先 `MailAddress.TryCreate` 再过 async 守卫，全部解析到私有地址或解析失败即拒绝。 |
| 2026-08-11 | **P5-04 完成**（定时采集，commit `7298b20`）：新增 `ScheduledCollectionPlan`（含 `Validate()` 与纯函数 `IsDueAt`）、`ScheduledCollectionCoordinator`、`IScheduledCollectionHistory` 与 Worker 侧 `ScheduledBiddingCollectionService`（`PeriodicTimer` 每分钟轮询，启动即跑一次，单次异常隔离不拖垮宿主）。幂等键为 (计划 ID, 执行日期)，`MarkNotifiedAsync` 只在推送成功后调用；校验发生在占位**之前**，避免坏配置烧掉当日槽位。全链路纵切打通：定时触发 → mock 采集 → 台账去重 → 渲染 → 通道推送。 |
